...
by at least x numbers. For example: if arr is [4, 10, 2, 6, 3] then your program should return the string true. The first 
range of numbers are 4, 5, 6, 7, 8, 9, 10 and the second range of numbers are 2, 3, 4, 5, 6. The last element in the array 
is 3, and there are 3 numbers that overlap between both ranges: 4, 5, and 6. If both ranges do not overlap by at least x 
...
(function OverlappingRanges(arr) { 
    ...
    for(var i = arr[0]; i <= arr[1]; i++){
        if(numObj[i] == undefined) numObj[i] = i
    }
    for(var i = arr[2]; i <= arr[3]; i++){
        if(numObj[i] != undefined) count += 1 
    }
